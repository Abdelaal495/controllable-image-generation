...
EOF
# ------------------------------------------------------------------ cluster.env
# Written once so submit.sh never needs the account or GPU type on the command line, and
# so no TRACKED file has to be edited per cluster (which would conflict on git pull).
if [ ! -f "$REPO_DIR/cluster.env" ]; then
  case "$CLUSTER" in
    narval)       DEFAULT_GPU="a100:1" ;;
    nibi|rorqual) DEFAULT_GPU="h100:1" ;;
    *)            DEFAULT_GPU="1" ;;
  esac
  DEFAULT_ACCOUNT=""
  if [ -d "$HOME/projects" ]; then
    n_found=$(ls -1 "$HOME/projects" 2>/dev/null | grep -cE '^(def|rrg|ctb)-' || true)
    if [ "$n_found" = "1" ]; then
      DEFAULT_ACCOUNT=$(ls -1 "$HOME/projects" | grep -E '^(def|rrg|ctb)-' | head -1)
    fi
  fi
  cat > "$REPO_DIR/cluster.env" <<EOF
# Generated by setup_cluster.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ) for cluster ${CLUSTER}.
# Gitignored: edit freely, it will never conflict with a git pull.
MPCFLOW_ACCOUNT=${DEFAULT_ACCOUNT}
MPCFLOW_GPU=${DEFAULT_GPU}
...
EOF
  echo
  echo "  Wrote cluster.env  (account='${DEFAULT_ACCOUNT:-UNSET -- edit cluster.env}', gpu=${DEFAULT_GPU})"
fi
...
echo
echo " Then -- batch:        bash submit.sh"
echo "         job array:    bash submit.sh --array 8"
echo "         (account and GPU come from cluster.env; no tracked file is edited)"
echo "=============================================================================="
