...
gitignored and ~100 GPU-hours to regenerate) can run `scripts/hpo_stage2.py` directly:

    python scripts/hpo_stage2.py --run results/stage1/hpo_full --run results/stage1/hpo_full_imf \
        --out configs/experiments_hpo_stage2.yaml

`scripts/hpo_stage2.py` reads only these CSVs.  Rows with `status != ok` are ignored by it; a snapshot
